...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    try {
      const { firstname, lastname, rank, dob, joindate, stationid } = req.body
      const data = await pool.query(`INSERT INTO Police(FirstName,LastName,Rank,DOB,JoinDate,StationID) VALUES ('${firstname}','${lastname}','${rank}','${dob}','${joindate}',${stationid});`);
